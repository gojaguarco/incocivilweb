      ...
      <div className="flex flex-col lg:flex-row w-full gap-14 items-end max-w-screen-xl">
        ...
        <div className="w-full lg:w-2/5 flex flex-col">
          <div className="w-full flex justify-end">
            <LinkButton
              color={content.ctaButton.color}
              link={content.ctaButton.link}
              size={content.ctaButton.size}
          ...
          <div className="flex flex-col w-full gap-4">
            ...
            {content.secondaryServices.map((service, index) => (
              <article
                className="w-full flex gap-4 p-2 rounded-lg bg-light el-shadow"
                key={index}
              >
                {service.imageObject && (
                  <Image
                    className="w-28 aspect-square object-cover rounded-md"
                    src={urlFor(service.imageObject)
                      .width(400)
                      .height(400)
                      .format("webp")
                      .url()}
                    alt={service.imageObject.alt}
                    width={400}
                    height={400}
                  />
                )}
                <div className="w-full flex flex-col justify-center gap-1 pr-1 lg:pr-4">
                  ...
                  <div className="pt-1">
                    <LinkButton
                      color="claro"
                      size="pequeño"
                      link={`/services/${service.reference._ref}`}
                      text="Ver Servicio"
                    />
                  </div>
                </div>
