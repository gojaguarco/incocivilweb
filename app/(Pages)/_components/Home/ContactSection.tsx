        ...
        <header className="w-full lg:w-1/2 flex flex-col gap-4">
          ...
          {content.imageObject && (
            <Image
              ...
              src={urlFor(content.imageObject)
                .width(800)
                .height(400)
                .format("webp")
                .url()}
          ...
          <div className="flex gap-5">
            <article className="w-1/2 p-4 rounded-xl bg-light flex flex-col items-end gap-3 justify-between el-shadow">
              <p className="w-full text-base font-normal">
                {content.homeProject.text}
              </p>
              <LinkButton
                color={content.homeProject.ctaButton.color}
                link={content.homeProject.ctaButton.link}
                size={content.homeProject.ctaButton.size}
                text={content.homeProject.ctaButton.text}
              />
            </article>
            <article className="w-1/2 p-4 rounded-xl bg-light flex flex-col items-end gap-3 justify-between el-shadow">
              <p className="w-full text-base font-normal">
                {content.comercialProject.text}
              </p>
              <LinkButton
                color={content.comercialProject.ctaButton.color}
                link={content.comercialProject.ctaButton.link}
                size={content.comercialProject.ctaButton.size}
                text={content.comercialProject.ctaButton.text}
              />
            </article>
          </div>
        </header>
        <ContactCard
          className="w-full lg:w-1/2 bg-light"
          content={content.contactCard}
          adminData={adminData}
        />
      </div>
    </section>
