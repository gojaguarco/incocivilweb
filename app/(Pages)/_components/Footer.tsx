...
const Footer = async () => {
  ...
  function formatPhoneNumber(phoneNumber: number): string {
    ...
    const formatted = `${phoneStr.slice(0, 3)} ${phoneStr.slice(3, 6)} ${phoneStr.slice(6)}`;
  
    return formatted;
  }

  const SocialLogo = ({ red }: { red: string }) => {
    if (red === "facebook") return (
    ...
    if (red === "X") return (
      <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="#fff"  className="icon icon-tabler icons-tabler-filled icon-tabler-brand-x"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8.267 3a1 1 0 0 1 .73 .317l.076 .092l4.274 5.828l5.946 -5.944a1 1 0 0 1 1.497 1.32l-.083 .094l-6.163 6.162l6.262 8.54a1 1 0 0 1 -.697 1.585l-.109 .006h-4.267a1 1 0 0 1 -.73 -.317l-.076 -.092l-4.276 -5.829l-5.944 5.945a1 1 0 0 1 -1.497 -1.32l.083 -.094l6.161 -6.163l-6.26 -8.539a1 1 0 0 1 .697 -1.585l.109 -.006h4.267z" /></svg>
      );
    if (red === "WhatsApp") return (
      <svg xmlns="http://www.w3.org/2000/svg" width={24} height={24} viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="icon icon-tabler icons-tabler-outline icon-tabler-brand-whatsapp">
        <path stroke="none" d="M0 0h24v24H0z" fill="none" />
        <path d="M3 21l1.65 -3.8a9 9 0 1 1 3.4 2.9l-5.05 .9" />
        <path d="M9 10a.5 .5 0 0 0 1 0v-1a.5 .5 0 0 0 -1 0v1a5 5 0 0 0 5 5h1a.5 .5 0 0 0 0 -1h-1a.5 .5 0 0 0 0 1" />
      </svg>
      );
    if (red === "Instagram") return (
    ...
    if (red === "YouTube") return (
      <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="#fff"  className="icon icon-tabler icons-tabler-filled icon-tabler-brand-youtube"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M18 3a5 5 0 0 1 5 5v8a5 5 0 0 1 -5 5h-12a5 5 0 0 1 -5 -5v-8a5 5 0 0 1 5 -5zm-9 6v6a1 1 0 0 0 1.514 .857l5 -3a1 1 0 0 0 0 -1.714l-5 -3a1 1 0 0 0 -1.514 .857z" /></svg>
    );
    if (red === "TikTok") return (
      ...
      <div className="max-w-screen-xl default-paddings w-full flex flex-col lg:flex-row justify-between z-20 py-16 gap-y-12">
        ...
        <section className="flex flex-col md:flex-row gap-x-20 gap-y-12 text-slate-300 px-6">
          ...
          <div className="flex flex-col gap-1.5">
            <h4>Información</h4>
            <div className="w-8 h-[3px] bg-accent1 rounded-full mb-3"/>
            {settings?.information.email && (<Link href="/">{settings.information.email}</Link>)}
            {settings?.information.phone && (<Link href="/">+57 {formatPhoneNumber(settings.information.phone)}</Link>)}
            {settings?.information.address && (<span>{settings.information.address}</span>)}
            {settings?.information.city && (<span>{settings.information.city}</span>)}
            {settings?.information.country && (<span>{settings.information.country}</span>)}
          </div>
          <div className="flex flex-col gap-1.5">
            <h4>Redes Sociales</h4>
            <div className="w-8 h-[3px] bg-accent1 rounded-full mb-3"/>
            <div className="flex">
              {settings?.socialLinks && settings.socialLinks.map((item, index) => {
