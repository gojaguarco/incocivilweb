...
const NavigationTypeButtons = ({currentId, idArray, surfaceTypeId}: TProps) => {
  ...
  return (
    <>
      <Link className="absolute top-1/2 -translate-y-1/2 left-4 bg-light p-2 rounded-full hover:bg-light-dark text-black" href={`/surface-type-surface/${surfaceTypeId}!${idArray[previousIndex()]}`}>
      ...
      <Link className="absolute top-1/2 -translate-y-1/2 right-4 bg-light p-2 rounded-full hover:bg-light-dark" href={`/surface-type-surface/${surfaceTypeId}!${idArray[nextIndex()]}`}>
...
export default NavigationTypeButtons