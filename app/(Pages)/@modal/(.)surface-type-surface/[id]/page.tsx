...
export default async function Page(props: { params: Promise<QueryParams> }) {
  ...
  const currentIndex = surfacesArray.indexOf(surfaceId)

  const nextIndex = () => {
    if (currentIndex === surfacesArray.length - 1) {
      return 0
    } else {
  ...
  return (
    <Modal>
      <article className="h-[80svh] sm:h-[70svh] md:h-[60svh] w-[90vw] max-w-screen-xl p-2 rounded-3xl bg-light relative overflow-hidden">
        <NavButtons backLink={`/surface-type-surface/${surfaceTypeId}!${surfacesArray[previousIndex()]}`} nextLink={`/surface-type-surface/${surfaceTypeId}!${surfacesArray[nextIndex()]}`} />
