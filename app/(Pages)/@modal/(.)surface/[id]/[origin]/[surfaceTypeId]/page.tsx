...
import {
  CATALOGUE_SURFACES_IDS_QUERY,
  HOMESURFACEIDS_QUERY,
  SURFACE_QUERY,
  SURFACES_QUERY,
  SURFACESBYTYPE_IDS_QUERY,
...
export default async function Page(props: { params: Promise<QueryParams> }) {
  ...
  const origin =
    (params.origin as "homepage" | "surfaceType" | "catalogue") || "";
  ...
  const backUrl =
    origin === "catalogue" && surfaceTypeId !== "0"
      ? `/catalogo?surfaceType=${surfaceTypeId}`
      : "/catalogo";
  ...
  return (
    <Modal backUrl={backUrl}>
      <article className="h-[80svh] sm:h-[70svh] md:h-[60svh] w-[90vw] max-w-screen-xl p-2 rounded-3xl bg-light relative overflow-hidden">
        {surfacesArray && (
          <NavButtons
            backLink={`/surface/${surfacesArray[previousIndex()]}/${origin}/${
              surfaceTypeId ?? "0"
            }`}
            nextLink={`/surface/${surfacesArray[nextIndex()]}/${origin}/${
              surfaceTypeId ?? "0"
            }`}
          />
        )}
