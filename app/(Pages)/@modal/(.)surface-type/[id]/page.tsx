...
import {
  ...
  SURFACESBYTYPE_QUERY,
  SURFACETYPEBYID_QUERY,
...
export default async function Page(props: { params: Promise<QueryParams> }) {
  ...
  const currentIndex = surfaceTypesArray.findIndex(
    (surface) => surface._id === id
  );

  const nextIndex =
    currentIndex === -1
      ? -1
      : surfaceTypesArray && currentIndex === surfaceTypesArray.length - 1
      ? 0
      : currentIndex + 1;
  ...
  const previousIndex =
    currentIndex === -1
      ? -1
      : surfaceTypesArray && currentIndex == 0
      ? surfaceTypesArray.length - 1
      : currentIndex - 1;

  const prevId =
    (surfaceTypesArray[previousIndex]?._id &&
      surfaceTypesArray[previousIndex]._id) ||
    surfaceTypesArray[surfaceTypesArray.length - 1]?._id;
  const nextId =
    (surfaceTypesArray[nextIndex]?._id && surfaceTypesArray[nextIndex]._id) ||
    ...
    <Modal>
      <article className="relative no-scrollbar bg-light-dark z-10 gap-2 md:gap-10 p-3 sm:p-5 md:p-8 flex flex-col w-full rounded-2xl max-h-[90vh] overflow-y-scroll">
        <NavButtons
          ...
          backLink={prevId && `/surface-type/${prevId}`}
          nextLink={nextId && `/surface-type/${nextId}`}
        />
        <div className="flex flex-col lg:flex-row gap-2 md:gap-10">
          <div className="relative w-full">
            <NavButtons
              ...
              nextLink={nextId && `/surface-type/${nextId}`}
            />
            {surfaceType.imageObject && (
              <Image
                className="w-full object-cover rounded-lg h-40 lg:h-auto"
                src={urlFor(surfaceType.imageObject)
                  .width(800)
                  .height(400)
                  .format("webp")
                  .url()}
                alt={surfaceType.imageObject.alt}
                width={800}
                height={400}
              />
            )}
